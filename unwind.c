...
static void DUMP_BYTES(int fd, off_t offset, uint32_t len)
{
    if (len > 0x80) {
        len = 0x80;
        DEBUG("Cutting debug output: Too long dump [%d bytes]\n", len);
    }

    off_t initial_off = offset;
    DEBUG("DUMP[%08llx]: ", offset);
                        ...
                        void *start_rip, const void const *process_rip)
{
    ...
    *stack_offset = cie_cfa_offset;
    ...
    DEBUG("DW_CFA decode start [len = %d]\n", cfa_len);
    while (offset < cfa_end) {
        ...
        case DW_CFA_nop:
            ...
            break;
        default:
            DEBUG("Unrecognized CFA opcode: 0x%02x@0x%p\n", opcode, (void *)offset);
...
void decode_cie(int fd, uint32_t offset, uint32_t cie_len)
{
    ...
    //DEBUG("Decoding CIE CFA at %p + 0x%04x\n", (void *)(uint64_t)offset, cfa_opcodes_len);
    decode_cfa_opcodes(fd, offset, cfa_opcodes_len, &cie_cfa_offset, NULL, NULL);
    //DEBUG("CIE CFA initial offset: %d bytes\n", cie_cfa_offset);
               ...
               uint32_t fde_len, int32_t *stack_offset)
{
    ...
    if (process_rip >= fde_rip_start && process_rip < (fde_rip_start + fde_off)) {
        DEBUG("0x%08x +0x%04x [ID: %x LEN:%x] Looking for %p\n", (uint32_t)(uint64_t)fde_rip_start, fde_off, 
                                                                 fde_id, fde_len, process_rip);
        DEBUG("%p %p %p\n", fde_rip_start, process_rip, fde_rip_start + fde_off);
        /* Found the fde belonging to this frame */
        DEBUG("Found FDE: 0x%08x +0x%04x\n", fde_rip_start, fde_off);
...
int32_t unwind_find_caller_offset(pid_t pid, void *rip)
{
    ...
    DEBUG("Analyzing total of %d FDEs\n", (int)initial_fde_count);
    ...
    DEBUG("Couldn't find right FDE for IP: %p\n", (void *)rip);
    EXIT_WITH_FAILURE_STR("Don't know how to continue");
    return 0;
}
...
int unwind_prepare(char *file)
{
    ...
    for (int i = 0; i < ehdr.e_phnum; i++) {
        ...
        case PT_GNU_EH_FRAME:
            ...
            DEBUG("Valid .eh_frame_hdr section found in address: %p\n", (void *)phdr.p_offset);
            DEBUG(".eh_frame contains %d entries\n", eh_frame_hdr_data.fde_count);
            ...
            DEBUG(".eh_frame should be at %p\n", (void *)absolute_contents_off);
